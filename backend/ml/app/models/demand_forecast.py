...
def predict_demand(features: List[float]) -> Optional[float]:
    ...
    if _model_cache is None:
        with _cache_lock:
            # Double-checked: concurrent predictions serialize here so a cache
            # miss only triggers one auto-train and the cache is only ever
            # populated from a fully published, valid generation.
            if _model_cache is None:

...
The geofence endpoint requires a driver JWT (HS256, signed with `JWT_SECRET`) sent as `Authorization: Bearer <jwt>`. A `driver` role may only query their own location (`sub` must equal `driver_id`); querying another driver's location returns `403`. Operator roles (`admin`, `operator`, `dispatcher`) may query any driver. Requests without a valid token return `401`; if `JWT_SECRET` is unset the endpoint fails closed (`503`).
